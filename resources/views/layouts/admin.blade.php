                ...
                <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                    ...
                    <div style="max-height: 300px; overflow-y: auto;">
                        @forelse(Auth::user()->unreadNotifications as $notification)
                            <a href="{{ route('notifications.read', $notification->id) }}" class="dropdown-item" style="white-space: normal;">
                                <i class="fas fa-envelope mr-2 text-primary"></i> 
                                <span class="text-sm font-weight-bold">{{ $notification->data['task_number'] ?? 'Task Update' }}</span>
                                <br>
                                <small class="text-muted ml-4">{{ \Illuminate\Support\Str::limit($notification->data['message'] ?? '', 40) }}</small>
                                <br>
                                <small class="text-muted float-right" style="font-size: 10px;">{{ $notification->created_at->diffForHumans() }}</small>
                            </a>
                            <div class="dropdown-divider"></div>
                        @empty
                            <a href="#" class="dropdown-item text-center text-muted small">No New Notifications</a>
                        @endforelse
                    </div>
                    
                    <div class="dropdown-divider"></div>
                    <a href="{{ route('notifications.readAll') }}" class="dropdown-item dropdown-footer text-center">Mark All as Read</a>
                </div>
            </li>
        ...
        <div class="sidebar">
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                    ...
                    {{-- 2. ADMINISTRATION (Restricted to Owner/Admin) --}}
                    ...
                    {{-- 3. HR MANAGEMENT --}}
                    @canany(['employee-list', 'department-list', 'designation-list', 'shift-list'])
                    <li class="nav-item has-treeview {{ request()->routeIs(['employees.*', 'designations.*', 'departments.*', 'shifts.*', 'shift-assignments.*']) ? 'menu-open' : '' }}">
                        <a href="#" class="nav-link">
                    ...
                    {{-- 4. EMPLOYEE PORTAL (Visible if user is linked to an employee) --}}
                    @if(Auth::user()->employee)
                    <li class="nav-item has-treeview {{ request()->routeIs(['leave-requests.create', 'salaries.my-history', 'attendances.my', 'tasks.my']) ? 'menu-open' : '' }}">
                        <a href="#" class="nav-link">
                        ...
                        <ul class="nav nav-treeview">
                            ...
                            <li class="nav-item"><a href="{{ route('attendances.my') }}" class="nav-link"><i class="far fa-circle nav-icon"></i><p>My Attendance</p></a></li>
                            <li class="nav-item"><a href="{{ route('salaries.my-history') }}" class="nav-link"><i class="far fa-circle nav-icon"></i><p>My Payslips</p></a></li>
                            <li class="nav-item"><a href="{{ route('tasks.my') }}" class="nav-link"><i class="fas fa-clipboard-list nav-icon"></i><p>My Tasks</p></a></li>
                        </ul>
                    </li>
                    @endif

                    {{-- 5. PAYROLL MANAGEMENT --}}
                    @canany(['salary-sheet-list', 'loan-list', 'fund-list', 'salary-component-list', 'tax-rate-list'])
                    <li class="nav-item has-treeview {{ request()->routeIs(['salaries.*', 'loans.*', 'funds.*', 'salary-components.*', 'tax-rates.*', 'payrolls.history']) ? 'menu-open' : '' }}">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-file-invoice"></i>
                            <p>Payroll Management<i class="right fas fa-angle-left"></i></p>
                        </a>
                        <ul class="nav nav-treeview">
                    ...
                    {{-- 6. LEAVE & ATTENDANCE --}}
                    @canany(['attendance-list', 'leave-type-list', 'leave-request-list', 'holiday-list'])
                    <li class="nav-item has-treeview {{ request()->routeIs(['attendances.*', 'holidays.*', 'leave-types.*', 'leave-requests.index', 'leave-encashments.index']) ? 'menu-open' : '' }}">
                        <a href="#" class="nav-link">
                        ...
                        <ul class="nav nav-treeview">
                            ...
                            <li class="nav-item"><a href="{{ route('leave-requests.index') }}" class="nav-link {{ request()->routeIs('leave-requests.index') ? 'active' : '' }}"><i class="far fa-circle nav-icon"></i><p>Leave Requests</p></a></li>
                            @endcan
                            @can('leave-encashment-list')
                    ...
                    {{-- 7. TASKS MANAGEMENT --}}
                    @canany(['task-list', 'task-create', 'recurring-task-list', 'task-report', 'task-category-list'])
                    <li class="nav-item has-treeview {{ request()->routeIs(['tasks.*', 'recurring-tasks.*', 'task-categories.*']) ? 'menu-open' : '' }}">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-tasks"></i>
                            <p>Tasks Management<i class="right fas fa-angle-left"></i></p>
                        </a>
                        <ul class="nav nav-treeview">
                            @can('task-list')
                            <li class="nav-item">
                                <a href="{{ route('tasks.index') }}" class="nav-link {{ request()->routeIs('tasks.index') ? 'active' : '' }}">
                                    ...
                                    <p>Task Board
                                        ...
                                        @if($pendingCount > 0) <span class="badge badge-warning right">{{ $pendingCount }}</span> @endif
                                    </p>
                                </a>
                            </li>
                            @endcan
                            @can('recurring-task-list')
                            <li class="nav-item">
                                <a href="{{ route('recurring-tasks.index') }}" class="nav-link {{ request()->routeIs('recurring-tasks.*') ? 'active' : '' }}">
                                    <i class="far fa-circle nav-icon"></i><p>Recurring Rules</p></a>
                            </li>
                            @endcan
                            @can('task-report')
                            <li class="nav-item">
                                <a href="{{ route('tasks.report') }}" class="nav-link {{ request()->routeIs('tasks.report') ? 'active' : '' }}">
                                    <i class="far fa-circle nav-icon"></i><p>Task Analytics</p></a>
                            </li>
                            @endcan
                            @can('task-category-list')
                            <li class="nav-item">
                                <a href="{{ route('task-categories.index') }}" class="nav-link {{ request()->routeIs('task-categories.*') ? 'active' : '' }}">
                                    <i class="far fa-circle nav-icon"></i><p>Task Categories</p></a>
                            </li>
                            @endcan
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('tasks.client_requests') }}">
                    ...
                    {{-- 10. SYSTEM REPORTS --}}
                    @canany(['salary-sheet-list', 'attendance-list', 'leave-request-list', 'loan-list', 'fund-list'])
                    ...
                    {{-- 11. CREDENTIALS & SERVICES (Admin Only) --}}
                    @hasanyrole('Owner|Admin')
                    <li class="nav-header">ADMIN SERVICES</li>
                    <li class="nav-item has-treeview {{ request()->routeIs('client-credentials.*') ? 'menu-open' : '' }}">
                        <a href="#" class="nav-link">
                            <i class="nav-icon fas fa-key"></i>
                            <p>Credentials Vault<i class="right fas fa-angle-left"></i></p>
                        </a>
                        <ul class="nav nav-treeview">
